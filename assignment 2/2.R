...
#idw_mask$predicted <- ifelse(idw_mask$predicted > 8571428, "High", "Other")


# 定义分类的阈值和标签
breaks <- c(-Inf, 8571428, Inf)
labels <- c("Other", "High")

# 对 predicted 层进行分类
classification <- classify(idw_mask, breaks, labels)

# 将分类后的栅格转换为数据框用于绘图
classification_df <- as.data.frame(classification, xy = TRUE)


# 绘制地图
ggplot() +
  geom_raster(data = classification_df, aes(x = x, y = y, fill = layer)) +
  geom_sf(data = country_boundary, fill = NA, color = NA) +
  scale_fill_manual(values = c("Other" = "green", "High" = "blue")) +
  labs(fill = "Prediction Category", 
       title = "Prediction Value Distribution Map",
       x = "Longitude", 
       y = "Latitude") +
  theme_minimal() +
